...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Image caption quality assessment tool')
...
def process_single_evaluation(data_item, args, prompt_template):
    ...
    key_points = data_item.get('key_point', [])
    key_points_str = "\n".join(key_points)
    
    modified_prompt = prompt_template.replace("{key_points}", key_points_str).replace("{answer}", answer)
    
    response_gpt = gpt_evaluate(modified_prompt, args.base_url, args.api_key, args.model, args.max_retries)
    ...
    try:
        ...
        print(f"Error processing image {data_item['id']}: {e}")
        return None
...
def process_batch_evaluation(batch_data, args, prompt_template, output_file):
    ...
    with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
        future_to_data = {
            executor.submit(process_single_evaluation, data_item, args, prompt_template): data_item
...
def calculate_scores_and_save(output_path):
        ...
        for line in file:
            try:
                ...
                if 'restriction' in data and len(data['restriction']) > 1:
                    ...
                    if 'answer' in data:
                        answer = data['answer']
                        if not check_length_requirement(data, len_limit, answer, 'content'):
                            score = max(score - 1, 0)
    ...
    with open(summary_path, 'w', encoding='utf-8') as summary_file:
        ...
        summary_file.write(f"Total evaluated images: {total_entries}\n")
        summary_file.write(f"Valid evaluated images: {valid_entries}\n")
        ...
        for dimension, scores in dimension_scores.items():
            ...
            summary_file.write(f"\nDimension: {dimension} ({dim_count} images)\n")
        ...
        with open(args.data_path, 'r', encoding='utf-8') as file:
            ...
            for i, line in enumerate(lines):
                ...
                if len(batch_data) >= args.batch_size or i == len(lines) - 1:
                    print(f"Processing batch: {len(batch_data)} images")
                    process_batch_evaluation(batch_data, args, prompt_template, output_file)
                    batch_data = []
