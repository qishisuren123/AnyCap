...
def parse_arguments():
    ...
    parser.add_argument('--batch_size', type=int, default=5,
                        help='Batch size, number of videos processed simultaneously')
...
def gpt_evaluate(prompt, client, model, max_retries=5):
...
def process_single_evaluation(data_item, client, args, prompt_template):
    """Process evaluation for a single data item"""
    ...
    response_gpt = gpt_evaluate(modified_prompt, client, args.model, args.max_retries)
...
def process_batch_evaluation(batch_data, client, args, prompt_template, output_file):
    """Process batch evaluation using ThreadPoolExecutor"""
    results = []
    
    with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
        # Submit all tasks
        future_to_data = {
            executor.submit(process_single_evaluation, data_item, client, args, prompt_template): data_item
...
def process_sync_evaluation(args):
    """Process evaluation using synchronous approach"""
    ...
    try:
        ...
        print(f'Loaded predefined evaluation points: {len(requirements_data)} items')
    except FileNotFoundError:
        print(f"Error: Cannot find predefined evaluation points file {args.content_data_path}")
        return
    except json.JSONDecodeError:
        print(f"Error: Incorrect format in predefined evaluation points file")
        return
        ...
        with open(args.data_path, 'r', encoding='utf-8') as file:
            ...
            for i, line in enumerate(lines):
                ...
                if len(batch_data) >= args.batch_size or actual_index == len(lines) - 1:
                    ...
                    process_batch_evaluation(batch_data, client, args, prompt_template, output_file)
                    
                    batch_data = []
                    batch_indices = []
    
    print('Completed synchronous batch evaluation, starting score calculation.')
...
if __name__ == "__main__":
    ...
    if args.calculate_only:
        ...
        print("Using synchronous batch processing mode for evaluation...")
        process_sync_evaluation(args)