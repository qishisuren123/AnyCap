...
def load_responses_from_jsonl(file_path):
    """Load pre-generated responses from JSONL file"""
    responses = {}
    try:
...
def get_most_available_gpu():
    ...
    # Sort by remaining memory (simplified handling)
...
def process_data(args):
    ...
    with open(args.output_path_style, 'w', encoding='utf-8') as outfile2_style, \
         open(args.output_path_content, 'w', encoding='utf-8') as outfile2_content:
        for line in tqdm(lines, desc="Processing JSONL", unit="line"):
            ...
            if id not in pregenerated_responses:
                print(f"Warning: Video ID {id} not found in pre-generated responses, skipping this entry")
                continue
            
            init_response = pregenerated_responses[id]
            ...
            with torch.no_grad():
                question_align = f"{video_prefix}You are a multimodal aligner, and your task is to transform the existing caption into a caption that better meets the requirements. The requirement is: '{question}'.The existing caption is: '{init_response}'. You only need to answer the caption that better meets the requirements, and do not add anything else."
                question_align = [question_align]
                response = model.batch_chat_video(
                    tokenizer=tokenizer,
                    pixel_values=pixel_values,
                    questions=question_align,
            ...
            if restriction in content_restriction:
                ...
                print(f'restriction "{restriction}" is not in the storage list, cannot process.')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-segments", type=int, default=8, help="Number of video frames to sample") 
